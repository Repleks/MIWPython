...
for i, criterion in enumerate(criteria):  # Pętla po kryteriach
    for j, max_depth in enumerate(max_depths):  # Pętla po maksymalnych głębokościach
        ...
        clf = DecisionTreeClassifier(criterion=criterion, max_depth=max_depth, random_state=42)  # Tworzy instancję klasyfikatora drzewa decyzyjnego
        clf.fit(X_train, y_train)  # Trenuje klasyfikator na danych treningowych
        ...
        train_accuracy = accuracy_score(y_train, y_pred_train)  # Oblicza dokładność klasyfikacji na danych treningowych
        test_accuracy = accuracy_score(y_test, y_pred_test)  # Oblicza dokładność klasyfikacji na danych testowych
