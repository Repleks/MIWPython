...
log_reg = LogisticRegression(random_state=42)  # Inicjalizuje klasyfikator regresji logistycznej
log_reg.fit(X_train, y_train)  # Dopasowuje model do danych treningowych
log_reg_train_accuracy = accuracy_score(y_train, log_reg.predict(X_train))  # Oblicza dokładność na danych treningowych
log_reg_test_accuracy = accuracy_score(y_test, log_reg.predict(X_test))  # Oblicza dokładność na danych testowych

# Krok 4: Trenowanie klasyfikatora SVM default
svm_clf = SVC(random_state=42)  # Inicjalizuje klasyfikator SVM
svm_clf.fit(X_train, y_train)  # Dopasowuje model do danych treningowych
svm_train_accuracy = accuracy_score(y_train, svm_clf.predict(X_train))  # Oblicza dokładność na danych treningowych
svm_test_accuracy = accuracy_score(y_test, svm_clf.predict(X_test))  # Oblicza dokładność na danych testowych
